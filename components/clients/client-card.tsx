    ...
    <Card className="animate-fade-in hover:shadow-md transition-shadow">
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span>{client.educationalBackground}</span>
        </div>

        {client.interestedCourses.length > 0 && (
          <div className="flex items-center gap-2 text-sm">
            <BookOpen className="h-4 w-4 text-muted-foreground" />
            <span>{client.interestedCourses[0].courseName}</span>
          </div>
        )}
        ...
        <div className="flex items-center justify-between pt-2">
          {client.totalValue > 0 ? (
            <span className="text-sm font-medium">Valor: {formatCurrency(client.totalValue)}</span>
          ) : (
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" asChild>
              <Link href={`/leads/${client.id}`}>Ver Detalhes</Link>
            </Button>
          </div>
        </div>
      </CardContent>
